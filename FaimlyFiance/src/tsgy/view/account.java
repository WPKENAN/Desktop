...
public class account extends JFrame
{
	...
	String[] Names={"ID","Name","Balance","Remark"};//创建表格中的横标题
	...
	table.setPreferredScrollableViewportSize(new Dimension(600,300));//  设置此表视口的首选大小
        ...
        b1.addActionListener(new ActionListener() {
            ...
            String fsex=t2.getText();
             String fs=t3.getText();
            AccountData md = new AccountData(fname,fsex,fs);
            int result=account.addAccount(md);
            ...
            //显示成员列表
            dtm.setRowCount(0);
            ResultSet rs=account.getAccount();
            try {
	    while(rs.next()){
	        Vector v=new Vector();
        ...
        b2.addActionListener(new ActionListener() {
            ...
            if(rowcount >= 0)
            {
                ...
                AccountData md = new AccountData(s1,s2,s3);
                 int result=account.delAccount(md);
		...
		dtm.addRow(v);
            ...
            String fsex1=t2.getText();
             String fs=t3.getText();
            AccountData md = new AccountData(fname1,fsex1,fs);
            t1.setText("");  
            t2.setText("");
            t3.setText("");
            dtm.setRowCount(0);
            rs =account.queryAccount(md);
            try {
	        while(rs.next()){
		Vector v=new Vector();
                 v.add(rs.getString("id"));
		...
		dtm.addRow(v);
            ...
            String s2=JOptionPane.showInputDialog(null, "请修改姓名", null,JOptionPane.PLAIN_MESSAGE);
            String s3=JOptionPane.showInputDialog(null, "请修改余额 ", null,JOptionPane.PLAIN_MESSAGE);
            String s4=JOptionPane.showInputDialog(null, "请修改备注 ", null,JOptionPane.PLAIN_MESSAGE);
            int result=account.alterAccount(s1,s2,s3,s4);
