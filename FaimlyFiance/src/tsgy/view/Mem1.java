 ...
 * @author xhj
 */
public class Mem1 extends JFrame
{
    JPanel panel1,panel2;
    JButton b1=new JButton("添加");
    JButton b2=new JButton("删除");
    JButton b3=new JButton("成员列表");
	...
	String[] Names={"ID","Familyname","sex"};//创建表格中的横标题
	...
	table.setPreferredScrollableViewportSize(new Dimension(600,300));//  设置此表视口的首选大小
        ...
        b1.addActionListener(new ActionListener() {
            ...
            member md = new member(fname,fsex);
            int result=memberdao.addMember(md);
            ...
            //显示成员列表
            dtm.setRowCount(0);
            ResultSet rs=memberdao.getMember();
            try {
	    while(rs.next()){
	        Vector v=new Vector();
        ...
        b2.addActionListener(new ActionListener() {
            ...
            if(rowcount >= 0)
            {
                 ...
                 member md = new member(s1,s2);
                 int result=memberdao.delMember(md);
		...
		dtm.addRow(v);
            ...
            member md = new member(fname1,fsex1);
            t1.setText("");  
            t2.setText("");
            
            dtm.setRowCount(0);
            rs =memberdao.queryMember(md);
            try {
	        while(rs.next()){
		Vector v=new Vector();
                v.add(rs.getString("id"));
		...
		dtm.addRow(v);
            ...
            String s3=JOptionPane.showInputDialog(null, "请修改性别", null,JOptionPane.PLAIN_MESSAGE);
            int result=memberdao.alterMember(s1,s2,s3);
